...
if [[ "$CONFIGURE_WEKA" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$RUN_TENANT" =~ ^[Yy]$ ]]; then
        ...
        bao write -f transit/keys/tenant1-key >/dev/null
        ...
        cat > "$WORK_DIR/tenant1_policy.hcl" <<'HCL'
...
HCL
        bao policy write tenant1 "$WORK_DIR/tenant1_policy.hcl" >/dev/null
        ...
        T1_ROLE_ID=$(bao read -field=role_id auth/approle/role/tenant1/role-id)
        T1_SECRET_ID=$(bao write -f -field=secret_id auth/approle/role/tenant1/secret-id)
        ...
        OLD_ACCESSORS=$(bao list -format=json auth/approle/role/tenant1/secret-id 2>/dev/null \
            | python3 -c "import json,sys; print('\n'.join(json.load(sys.stdin)))" 2>/dev/null || true)

        T1_NEW_SECRET_ID=$(bao write -f -field=secret_id auth/approle/role/tenant1/secret-id)
        print_ok "New secret_id generated"

        # Revoke each old accessor
        ...
        print_info "Rotating the transit key creates a new key version in OpenBao."
        print_info "WEKA then re-encrypts the filesystem's DEK with the new version."
        echo ""

        bao write -f transit/keys/tenant1-key/rotate >/dev/null
        ...
        if weka security kms rewrap 2>/dev/null; then
            ...
            print_info "The new key version is active in OpenBao; WEKA will use it on the next key operation."
        fi
        ...
        echo "  bao delete auth/approle/role/tenant1"
        echo "  bao write transit/keys/tenant1-key/config deletion_allowed=true"
        echo "  bao delete transit/keys/tenant1-key"
        print_divider
    fi
